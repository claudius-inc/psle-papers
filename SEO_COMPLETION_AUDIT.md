...
| Deploy pipeline protects SEO regressions | GitHub Pages build runs generate, local SEO audit, deploy, then live SEO audit | `.github/workflows/nuxtjs.yml`, `scripts/audit-pages-workflow.mjs`, `SEO_RUNBOOK.md` | Run `25626609288` passed after stricter generated and live asset stale-snippet audits |
...
4. Follow `SEO_REINDEX_PLAN.md`: request indexing for priority URLs and resubmit `https://sgexamhub.com/sitemap.xml` in Google Search Console.
5. Local shell execution is currently unavailable in this environment, so verification for the latest changes was done through GitHub connector and live web reads rather than local commands.
